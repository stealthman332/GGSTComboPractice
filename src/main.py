...
BTN_COLORS = {
    'P': '#ff00bb', 'K': '#00d4ff', 'S': '#00ff88', 
    'H': '#ff2a00', 'D': '#ffaa00', 'DIR': '#ffffff'
}

class TrainerApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("GGST Combo Trainer V1.6 - Early Spawning Rhythm")
    ...
    def calculate_rhythm_timeline(self):
        ...
        for i, step in enumerate(self.engine.current_combo["sequence"]):
            ...
            if i > 0: accumulated_frames += max_f if max_f > 0 else 30 
    ...
    # --- THE NEW EARLY-SPAWNING TIMELINE LOGIC ---
    def trigger_reset(self):
        self.state = "COUNTDOWN"
        # Start the timeline negative 150 frames (2.5 seconds) in the past!
        self.combo_timer = -150 
        self.engine.combo_step = 0
        self.engine.last_input_frame = self.current_frame
        self.canvas_bg = "#050508"
        
        self.calculate_rhythm_timeline() # Build the track immediately
    ...
    def trigger_result(self, success, message):
        ...
        self.result_timer = 0 # Track how long to show the result screen
    ...
    def on_key_release(self, event):
        key = event.keysym.lower()
        if key in self.keys_held: self.keys_held.remove(key)
    ...
    def process_state_machine(self):
        # We now drive everything by counting exact frames
        if self.state in ["COUNTDOWN", "PRACTICING"]:
            self.combo_timer += 1

        if self.state == "COUNTDOWN":
            if self.combo_timer < -110: self.overlay_text = "3"
            elif self.combo_timer < -70: self.overlay_text = "2"
            elif self.combo_timer < -30: self.overlay_text = "1"
                ...
                self.overlay_text = ""
                # Sync logic engine current frame exactly to Frame 0 of the rhythm track
                self.engine.last_input_frame = self.current_frame 
                
        elif self.state == "RESULT":
            self.result_timer += 1
            if self.result_timer > 90: # 1.5 seconds at 60fps
                self.canvas_bg = "#050508"
                self.state = "IDLE"
    ...
    def draw_rhythm_track(self):
        ...
        # Draw blocks even during COUNTDOWN so they slide in early!
    ...
    def draw_virtual_hitbox(self):
        ...
        buttons = [
            ...
            ('P', 400, base_y + 5, 20), ('S', 450, base_y - 10, 20), ('D', 500, base_y - 10, 20),
            ('K', 400, base_y + 50, 20), ('H', 450, base_y + 35, 20)
        ]
        self._draw_hardware_panel(buttons, "ARCADE HITBOX", is_round=True)

    def draw_virtual_keyboard(self):
        base_y = 160
        held_mapped = [self.engine.key_map[k] for k in self.keys_held if k in self.engine.key_map]
        buttons = [
            ('up', 250, base_y, 20), ('left', 205, base_y + 45, 20),
            ('down', 250, base_y + 45, 20), ('right', 295, base_y + 45, 20),
            ('P', 400, base_y, 20), ('K', 445, base_y, 20), ('S', 490, base_y, 20),
            ('H', 400, base_y + 45, 20), ('D', 445, base_y + 45, 20)
        ]
        self._draw_hardware_panel(buttons, "MECHANICAL KEYBOARD", is_round=False)

    def _draw_hardware_panel(self, buttons, title, is_round):
        held_mapped = [self.engine.key_map[k] for k in self.keys_held if k in self.engine.key_map]
        self.canvas.create_round_rect(160, 120, 550, 245, radius=15, fill="#151520", outline="#333", width=2)
        self.canvas.create_text(355, 135, text=title, font=("Consolas", 10, "bold"), fill="#555")

        for action, x, y, r in buttons:
            is_active = action in held_mapped
            base_color = "#222"
            glow_color = BTN_COLORS.get(action, "#fff") if action not in ['up','down','left','right'] else "#fff"
            ...
            if is_round:
                ...
                self.canvas.create_rectangle(x - r - 2, y - r - 2, x + r + 2, y + r + 2, fill="#333", outline="", borderwidth=2)
                self.canvas.create_rectangle(x - r, y - r, x + r, y + r, fill=color, outline=glow_color if is_active else "#000", width=2)
            
            label = action.upper() if action in ['P', 'K', 'S', 'H', 'D'] else ""
            if label:
                self.canvas.create_text(x, y, text=label, font=("Arial", 12, "bold"), fill="#000" if is_active else "#777")
    ...
    def open_settings(self):
        ...
        win.geometry("320x600") # Made taller for new option
        win.configure(bg=BG_COLOR)
        win.transient(self)
        win.grab_set() 
        
        # Visualizer Preference
        ...
        vis_frame.pack()
        tk.Radiobutton(vis_frame, text="Hitbox", variable=self.temp_vis, value="hitbox", bg=BG_COLOR, fg="white", selectcolor="#333").pack(side=tk.LEFT, padx=10)
        tk.Radiobutton(vis_frame, text="Keyboard", variable=self.temp_vis, value="keyboard", bg=BG_COLOR, fg="white", selectcolor="#333").pack(side=tk.LEFT, padx=10)

        tk.Label(win, text="-"*30, bg=BG_COLOR, fg="#333").pack(pady=10)
        
        # Keybinds
        tk.Label(win, text="KEYBINDS:", bg=BG_COLOR, fg=ACCENT_COLOR, font=("Arial", 10, "bold")).pack(pady=5)
        tk.Label(win, text="Click an action, press a physical key.", bg=BG_COLOR, fg="#888", font=("Arial", 8)).pack()
        ...
        for idx, action in enumerate(actions):
            tk.Label(frame, text=f"{action.upper()}:", bg=BG_COLOR, fg=TEXT_COLOR, font=("Arial", 10, "bold")).grid(row=idx, column=0, pady=4, sticky=tk.E)
            ...
            btn.grid(row=idx, column=1, padx=10, pady=4)
        ...
        save_btn.pack(pady=15, fill=tk.X, padx=20)
